...
./mvnw clean spring-boot:build-image \
  --projects "$BITCOIN_CLIENT_APP_NAME" \
  -DskipTests="$SKIP_TESTS" \
  -Dspring-boot.build-image.imageName="$BITCOIN_CLIENT_DOCKER_IMAGE_NAME"

./mvnw clean spring-boot:build-image \
  --projects "$BITCOIN_API_APP_NAME" \
  -DskipTests="$SKIP_TESTS" \
  -Dspring-boot.build-image.imageName="$BITCOIN_API_DOCKER_IMAGE_NAME"
